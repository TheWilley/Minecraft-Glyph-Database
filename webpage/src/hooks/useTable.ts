...
export default function useTable(data: ConvertedData, textureKey: keyof ConvertedData, query: string) {
    ...
    useEffect(() => {
        if (query) {
            ...
            if (result.length) setHighlightedArea({ x: result[0].gridLocation.y, y: result[0].gridLocation.x });
    ...
    return { highlightedArea, filteredData, handleHoverChange, resetHighlitedArea, scrollTo };
}